PILOT Technical Support Employee Examination

English Version

Section A. Advanced PILOT Knowledge Test

Number of questions: 40

Correct answers: one most accurate option

Time limit: 60 minutes

1. An administrator created a user and specified a login, password, and email address. The user logged in but cannot see any objects. What is the most likely explanation?
...
2. A user must be given access to only two objects out of twenty. What is the most appropriate method?
...
3. Several dispatchers need the same set of system permissions and access to the same list of objects. Which solution reduces the likelihood of errors?
...
4. A user needs to allow an external contractor to monitor three objects for a limited period without sharing a login and password. What is the best option?
...
5. A user is blocked in the administrative panel. Which statement is the most accurate?
...
6. A user has the required rights to objects but cannot see a particular section of the system. What should be checked first?
...
7. The correct IMEI was entered when creating an object, but the wrong device type was selected. What is the most likely result?
...
8. Packets from a device reach the server but are not linked to the created object. What should be checked first?
...
9. What is the main purpose of an object group compared with a tag?
...
10. An object was moved from one contract to another. What risk should be considered?
...
11. An additional module must be enabled for a contract. Where is the most logical place to do this?
...
12. After an object is created, no data appears even though the ID and device type are correct. What is the most rational troubleshooting order?
...
13. A temperature sensor was created using the field temp_1, but the device sends only temperature in raw points. What will happen?
...
14. What is the most reliable way to determine the field for a new sensor?
...
15. The device sends a voltage value of 14592 mV. The value must be displayed in volts. Which configuration is correct?
...
16. When is a formula preferable to a calibration table?
...
17. When is a calibration table preferable to a simple formula?
...
18. When does editing the calibration table of a new sensor become available?
...
19. A fuel sensor calibration table contains the following points: 0 → 0 L; 1000 → 20 L; 2000 → 50 L; 3000 → 80 L. If the input is between 1000 and 2000, what principle is used to determine the intermediate value?
...
20. A fuel sensor shows a plausible current value, but the report contains false refuelings and drains. What is the most justified conclusion?
...
21. The same raw voltage transformation is used for several object models. Which mechanism is intended for reusing common processing logic?
...
22. After changing a sensor formula, the current value became correct, but an old report for the previous month did not change. What action will most likely be required?
...
23. For the DIN1 discrete input, the states “Equipment Off” and “Equipment On” must be displayed. What is the most suitable solution?
...
24. An ignition sensor was created using DIN1. The points show DIN1=1, but the sensor displays “Off.” What should be checked first?
...
25. How do raw points differ from the displayed track?
...
26. A single jump of several kilometers followed by an immediate return is visible on the track. What is the most informative first step?
...
27. After applying a filter, the track looks correct. Which conclusion must not be made?
...
28. A user selected a history period but cannot see a track. Which combination of causes is the most realistic?
...
29. Why is it important to use the same time period and time zone when comparing a track and a report?
...
30. Movement is shown in the history, but the ignition sensor is always Off. Which statement is the most accurate?
...
29. The temperature report is empty even though temperature is present in the raw points. What is the most logical step?
...
30. An equipment-operation report is generated, but all intervals have zero duration. What should be checked?
...
31. What is the main difference between the Report Builder and running a predefined report?
...
32. A user changed the set of report columns but cannot see the required metric. What is the most likely explanation?
...
33. The same report must be sent automatically every morning. Which tool is most suitable?
...
34. A temperature notification is configured for objects with a specific tag. A new object has the required temperature sensor, but the notification does not trigger for it. What should be checked first?
...
35. An email notification is configured correctly and the event is recorded in the system, but the user does not receive the email. Which check is most appropriate?
...
36. An event must be displayed to an operator in the Control Room and an event-processing algorithm must be launched at the same time. Which option is most appropriate?
...
37. A geofence has been created, but the visit report for an object is empty. What should be checked?
...
38. The Notifications module is not enabled for the contract. The user configured a temperature sensor and expects an automatic email when the threshold is exceeded. Which statement is the most accurate?
...
In each answer, the employee must not merely list interface items, but must explain:
...
PILOT User-Side Tasks

1. Configuring a New Object Based on Actual Data

A new object has been added to the system. The device is already transmitting coordinates and the following parameters:
...
2. Selecting a Mileage Source

The client wants to see accurate vehicle mileage. The device transmits GPS coordinates, the terminal odometer, and CAN mileage. Explain:
...
3. Comparing Data in Points, Graphs, and Reports

The client reports that the temperature shown in the object card, in a history point, and in the report for the same moment is different. Describe possible reasons and the troubleshooting sequence. Consider:
...
4. Configuring Multiple Temperature Sensors

A refrigerated vehicle has three temperature sensors:
...
The client requires a report with the following columns:
...
6. Complex Notification with Multiple Conditions

The client wants a notification when all of the following conditions are true at the same time:
...
7. Route-Control Configuration

A vehicle must travel daily between a warehouse and a retail location only along an approved route. Describe how geofences, notifications, and reports can be used to control:
...
8. Creating a Contract for a New Client

A new contract must be created for a company with ten vehicles. Describe the complete setup procedure:
...
9. Moving Objects Between Contracts

Five vehicles must be moved from one contract to another. Their history and sensor settings must be preserved, the new user must receive access, and old users must lose access. Describe:
...
10. Configuring a Partner

A new partner is being created and will independently manage its contracts and users. Describe which parameters must be configured:
...
11. Prepayment and Financial Blocking

A contract uses prepayment. The client balance became negative, but some objects are still visible and continue transmitting data. Describe:
...
12. Contract and Object Configurations
...
13. Eco Driving Module

The client enabled Eco Driving and wants to evaluate drivers based on:
...
The client wants to send configuration and engine-blocking commands to the terminal. Describe:
...
15. GARM / Control Room Module

Operator work with critical events must be configured for:
...
Each answer must contain at least three meaningful parts:

What must be checked or configured.

In what sequence the actions should be performed.

How successful completion will be confirmed.

For situational questions, it is also desirable to specify:

possible causes;

data that must be requested from the client;

how the issue can be reproduced;

criteria for escalation to Line 2 support or the developers.
...
Section C. Practical Examination

The examinee completes the tasks in sequence. First, the examinee fully configures PILOT through the user and administrative interfaces. After completing the user-side configuration, the examinee proceeds to the test server, database, recalculations, and logs.

All actions must be performed in the test environment. For each task, the examinee must demonstrate the completed configuration to the examiner and explain the sequence of actions.

Part 1. Retransmission Configuration

1. Locate the Source Object
...
Part 5. Object Creation and Configuration
...
Part 9. Logistics Module

19. Configure the Object for Logistics
...
Part 10. Service Module

24. Configure Maintenance
...
Part 11. Commands Module

26. Engine Lock Command Template
...
Part 15. Test Server Operations
...
Part 16. Manual Recalculations

40. Locate Recalculation Jobs in crontab
...
Part 18. Retransmission Debugging

45. Enable Retransmission Debugging
